"""Run deterministic source-mutation recovery checks and emit commit-bound evidence."""
...
_TEST_MODULES = ("tests.test_mutation_recovery",)
...
def normalize_mutation_recovery_report(
    *, source_commit: str, scenarios: Mapping[str, bool]
) -> dict[str, Any]:
    """Retain only candidate-bound source-mutation recovery facts."""
    ...
    report = {
        "schema": 1,
        "suite": "mutation-recovery",
        "source_commit": source_commit,
        "checks": [
            "interrupted-text-rollback",
            "interrupted-structural-rollback",
            "idempotent-reopen",
        ],
...
def _run_recovery_tests() -> dict[str, bool]:
    ...
    runner = unittest.TextTestRunner(
        stream=io.StringIO(), verbosity=0, resultclass=_RecoveryResult
    )
