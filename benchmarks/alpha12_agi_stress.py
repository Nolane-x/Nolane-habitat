...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        mem = ws.memory_record('episodic','Alpha12 stress explored the supplied AGI corpus with bounded source paging and twin snapshots',agent_id=aid,confidence=.9,provenance={'corpus':src.name,'context_handle':exp['context_handle']})
        ...
        report = {
            ...
            'world_summary': world,
            'claim_boundary': 'Deterministic alpha.12 compiler/retrieval/effect/dataflow/memory/activity stress on the supplied corpus. Effect/Dataflow facts are static evidence, runtime correlation is not causal proof, and this is not an LLM/token/AGI benchmark.'
        }
