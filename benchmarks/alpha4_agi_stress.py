...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        t=time.perf_counter(); warm=ws.refresh('alpha4-stress-warm-noop'); warm_ms=round((time.perf_counter()-t)*1000,2)
        ctx=ws.orient('context engineering tool orchestration semantic proof',budget=12)
        ws.residency_configure(max_objects=8,max_source_bytes=30000)
        residency_admit=ws.residency_admit(ctx.handle,pin_top=2,max_admit=8)
        resident_packet=ws.residency_materialize(max_source_bytes=12000,max_objects=8)
        ...
        report={
          'release':'0.1.0-alpha.4','corpus':src.name,'cold_ingest_ms':cold_ms,'warm_refresh_ms':warm_ms,
          ...
          'residency':{'admitted':residency_admit['admitted'],'status':residency_admit['status'],'materialized_source_bytes':resident_packet['source_bytes']},
          'partition_summary':{'warm_base':warm['project_semantics']['base-resolver']},
