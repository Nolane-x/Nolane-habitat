...
def habitat_case(proto,task,label):
    ...
    packet=proto.handle({'id':'p','method':'workspace.context.prefetch','params':{'handle':ctx['handle'],'max_source_bytes':5000,'max_pages':6}})['result']
    ...
    return {'paths':[x['path'] for x in ctx['objects']],'confidence':ctx['decision_packet']['retrieval_confidence'],
            'abstain':ctx['decision_packet']['abstention_recommended'],'source_bytes':packet['source_bytes'],'page_count':len(packet.get('pages',[])),'trace':trace}
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        report={'release':'0.1.0-alpha.5','benchmark':'context-precision-plumbing-v2','fixture':{'files':ws.enter()['file_count'],'noise_files':args.noise_files},
                ...
                'claim_boundary':'Not a same-model agent A/B. It supports only retrieval-plumbing claims, not token, reasoning, speed, or coding-success claims.'}
        ws.close()
