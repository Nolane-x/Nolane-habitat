...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        enter=ws.enter(); coldp=ws.semantic_provider_report(); t=time.perf_counter(); warm=ws.refresh('alpha7-stress-warm'); warm_ms=round((time.perf_counter()-t)*1000,2)
        ...
        live=ws.semantic_provider_report(); ws.close()
        report={'release':RELEASE,'corpus':src.name,'cold_ingest_ms':cold,'warm_refresh_ms':warm_ms,
                ...
                'providers_cold':coldp,'providers_warm':live,'warm_refresh':warm,
                'explorer':{'confidence':exp['retrieval_confidence'],'region_count':exp['region_count'],'lines_selected':exp['lines_selected'],'paths':[r['path'] for r in exp['regions']],
                            'source_bytes_read':exp['source_bytes_read'],'faulted_source_bytes':fetched['source_bytes'],'efficiency':eff},
                ...
                'claim_boundary':'Measures deterministic alpha.7 semantic/compiler/explorer plumbing on the supplied corpus; not an LLM/token/AGI benchmark.'}
