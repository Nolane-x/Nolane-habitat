...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        enter=ws.enter(); coldp=ws.semantic_provider_report(); t=time.perf_counter(); ordinary=ws.reconcile(); ordinary_ms=round((time.perf_counter()-t)*1000,2); t=time.perf_counter(); warm=ws.refresh('alpha9-stress-deep-scrub'); warm_ms=round((time.perf_counter()-t)*1000,2)
        ...
        report={'release':RELEASE,'corpus':src.name,'cold_ingest_ms':cold,'warm_refresh_ms':warm_ms,
                ...
                'claim_boundary':'Measures deterministic alpha.9 integrity/index/compiler/explorer/I-O plumbing on supplied corpus; not an LLM/token/AGI benchmark. Authority I/O and model-visible source bytes are separate metrics.'}
