...
class MutationRecoveryTests(unittest.TestCase):
    ...
    def test_windows_file_aliases_cannot_bypass_an_exact_deny(self):
        with tempfile.TemporaryDirectory() as td:
            base = Path(td)
            project = base / "project"
            project.mkdir()
            ...
            try:
                ...
                case_decision = workspace.policy_evaluate("edit", path="version")["decision"]
                ...
                self.assertEqual("0.1.0-alpha.19\n", protected.read_text(encoding="utf-8"))
                self.assertEqual(
                    0,
                    workspace.store.conn.execute("SELECT COUNT(*) FROM transactions").fetchone()[0],
                )
    ...
    @unittest.skipUnless(os.name == "nt", "Win32 path alias semantics")
    def test_windows_case_aliases_share_one_path_lease_identity(self):
        with tempfile.TemporaryDirectory() as td:
            base = Path(td)
            project = base / "project"
            project.mkdir()
            (project / "VERSION").write_text("0.1.0-alpha.19\n", encoding="utf-8")
            workspace = HabitatWorkspace.create(project, base / "workspace")
            try:
